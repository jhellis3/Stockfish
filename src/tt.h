...
struct TTData {
    ...
    // clang-format off
    TTData(Move m, Value v, Value ev, Depth d, Bound b, bool pv, u8 r50count) :
        move(m),
        value(v),
        eval(ev),
        depth(d),
...
struct TTWriter {
   public:
    void write(Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, u8 generation8, u8 r50count);
