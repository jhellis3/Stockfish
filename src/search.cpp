...
void Search::Worker::iterative_deepening() {
    ...
    //Depth             lastBestMoveDepth = 0; // review
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        &&  eval >= beta)
    {
       ...
       if (    depth < (9 - 2 * ((ss-1)->mainLine || (ss-1)->secondaryLine || (ttData.move && !ttCapture)))
           ...
           return beta + (eval - beta) / 3; // review
   ...
   if (     ss->inCheck
    ...
    int lmrAdjustment =   cutNode * 2 // 3 + !ttData.move
                        + ttCapture
                        + ((ss+1)->cutoffCnt > 3)
                        - (!PvNode && doSingular)
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        Depth r = reduction(improving, depth, moveCount, delta) + ss->ttPv;
            ...
            &&  bestValue > VALUE_MATED_IN_MAX_PLY)
        {
            ...
            else if (!ss->followPV || !PvNode)
            {
                ...
                if (lmrDepth < 6 && history < -4097 * depth)
                    continue;
        ...
        r -= std::abs(correctionValue) / 26214400;
            ...
            && (!capture || lmrCapture))
        {
            ...
            if (value > alpha)
            {
                ...
                const bool doDeeperSearch = allowExt && d < newDepth && value > bestValue + 48;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 4) - (r > 5 && newDepth > 2), !cutNode);
        }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return ((reductionScale + 1133 - delta * 585 / rootDelta) >> 10) + !i * reductionScale / 2545;
}
