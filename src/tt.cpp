...
// TTEntry struct is the 16 bytes transposition table entry, defined as:
//
// key        32 bit
// depth      16 bit
// rule50      8 bit
...
//
// We use `bool(depth16)` as the cheap internal occupancy check, corresponding
...
struct TTEntry {
   ...
   private:
    ...
    RelaxedAtomic<u8>   r50count8;
    RelaxedAtomic<u16>  depth16;
...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, u8 curr_generation, u8 r50count) {
    ...
    if (b == BOUND_EXACT || u32(k) != key32 || d - DEPTH_NONE + 2 * pv > depth16 - 2
        || relative_age(curr_generation))
    {
        ...
        key32     = u32(k);
        r50count8 = u8(r50count);
        depth16   = u16(d - DEPTH_NONE);
             ...
             && Bound((genBound8 & BOUND_MASK) >> BOUND_SHIFT) != BOUND_EXACT)
    {
        ...
        if (std::abs(v16) < VALUE_INFINITE && is_decisive(v16))
            depth16 = std::max(int(depth16) - 1, 0);
    }
}
...
std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const {
    ...
    const u32      key32 = u32(key);  // Use the low 32 bits as key inside the cluster
    ...
    for (int i = 1; i < ClusterSize; ++i)
        if (replace->depth16 - 8 * replace->relative_age(generation8)
            > tte[i].depth16 - 8 * tte[i].relative_age(generation8))
            replace = &tte[i];

    return {false, TTData{Move::none(), VALUE_NONE, VALUE_NONE, DEPTH_NONE, BOUND_NONE, false, 0},
            TTWriter(replace)};
}
